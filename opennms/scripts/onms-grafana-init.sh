...
FLOW_DASHBOARD=$(curl -u "${GRAFANA_AUTH}" "http://grafana:3000/api/search?query=flow" 2>/dev/null | jq '.[0].url' | sed 's/"//g')
...
KEY_ID=$(curl -u "${GRAFANA_AUTH}" "${GRAFANA_URL}/api/auth/keys" 2>/dev/null | jq '.[] | select(.name="$(hostname)") | .id')
if [ "${KEY_ID}" != "" ]; then
...
GRAFANA_KEY=$(curl -u "${GRAFANA_AUTH}" -X POST -H "Content-Type: application/json" -d '{"name":"$(hostname)", "role": "Viewer"}' "${GRAFANA_URL}/api/auth/keys" 2>/dev/null | jq .key - | sed 's/"//g')
